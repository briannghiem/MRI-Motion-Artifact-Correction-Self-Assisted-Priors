...
def channel_attention(input_feature, ratio=8):
	...
	channel_axis = 1 if K.image_data_format() == "channels_first" else -1 #if channel_first, shouldn't channel_axis = 0?
...
def UNet(img_input):
	...
	#
	output = Conv2D(output_channels, (3, 3), data_format=IMAGE_ORDERING, padding='same')(deconv3)
...
def Correction_Multi_input(input_height, input_width):
	...
	#UNET
	input_channels = 2
	img_input_1 = Input(shape=(input_height, input_width, input_channels))
	img_input_2 = Input(shape=(input_height, input_width, input_channels))
	img_input_3 = Input(shape=(input_height, input_width, input_channels))
	# kk = 32*
	kk = 32
